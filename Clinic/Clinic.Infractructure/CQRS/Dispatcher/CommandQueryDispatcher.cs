...
internal class CommandQueryDispatcher : ICommandQueryDispatcher
{
    ...
    public async Task SendAsync(ICommand command, CancellationToken cancellationToken)
    {
        ...
        var wrapper = (AsyncCommandHandlerWrapper<ICommand>)Activator.CreateInstance(asyncCommandHandlerType, handler, _pipelineBuilder)!;
    ...
    public async Task<TResponse> SendAsync<TResponse>(IQuery<TResponse> query, CancellationToken cancellationToken)
    {
        ...
        var queryHandlerType = QueryHandlerType.MakeGenericType(queryType);
        var asyncQueryCommandHandler= AsyncQueryHandlerType.MakeGenericType(queryType);

        var handler = _services.GetService(queryHandlerType);
        var wrapper = (AsyncQueryHandlerWrapper<IQuery<TResponse>, TResponse>)Activator.CreateInstance(asyncQueryCommandHandler, handler, _pipelineBuilder)!;
        return await wrapper.HandleAsync(query: query, cancellationToken: cancellationToken);
    }


    private class AsyncCommandHandlerWrapper<TCommand> where TCommand : class, ICommand
    {
    ...
    private class AsyncQueryHandlerWrapper<TQuery, TResponse> 
